...
/* ─────────────────────────────────────────────────────────────
   ConsumerElectronicsSection.jsx
   – Real data from Supabase via React Query
   – Exact UI match to Figma
   – "Source now" navigates to /browse/electronics
───────────────────────────────────────────────────────────── */
...
const BANNER_IMAGE = "https://images.unsplash.com/photo-1546435770-a3e426bf472b?w=280&h=257&fit=crop";

/* ── thumbnail helper ── */
const getThumb = (p) => p.image_url || p.images?.[0] || null;

/* ── placeholder tiles shown while loading ── */
...
export default function ConsumerElectronicsSection() {
  ...
  /* Pad to exactly 8 cells so grid never collapses */
  ...
  return (
    <section className="w-full max-w-[1180px] mx-auto border border-gray-200 bg-white flex overflow-hidden">
      ...
      {/* ── Left Banner ── */}
      <div className="w-[270px] h-[257px] relative bg-[#EEF4F8] shrink-0 flex flex-col justify-between p-5">
        <img
          src={BANNER_IMAGE}
